...
def main():
    ...
    for filename in matches:
        ...
        with open(filename + '.html') as f:
            ...
            lines += format_titles(get_content(f.read()))
...
def get_title_line(filename):
    return '<span class="title0">' + filename.split('/')[-1] + '</span>'

def format_titles(lines):
    ret = []
    for line in lines:
        if '>###' in line:
            newline = line.split('>###')
            ret += ['<span class="title3">' + newline[1] + '</span>']
        elif '>##' in line:
            newline = line.split('>##')
            ret += ['<span class="title2">' + newline[1] + '</span>']
        elif '>#' in line:
            newline = line.split('>#')
            ret += ['<span class="title1">' + newline[1] + '</span>']
        else:
            ret += [line]
    return ret
